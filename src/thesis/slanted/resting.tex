...
\TODO{
\begin{itemize}
	\item Plot: maybe plot integrated KE because it's less harsh than max(abs(w))
	\item Conclusion: BTF meshes become severely distorted above steeper slopes and produce inaccurate or unstable results
	\item Conclusion: Slanted cells and cut cells can reduce spurious circulations over more gently-sloping terrain
	\item Conclusion: Spurious circulations worsen over steeper slopes when using slanted cells
	\item Conclusion: Even over the steepest slopes, spurious circulations remain modest on cut cell meshes and SLEVE meshes
	\item Conclusion: from this test seems like SLEVE or cut cells are the way forward, and definitely not slanted cells!  but what about with advection?  this motivations schaerWaves test
\end{itemize}
}

\TODO{
\begin{itemize}
	\item SLEVE does best in resting tests but we don't test SLEVE anywhere else
	\item this seems odd given SLEVE shows such promise (and because operational models use smoothed TF meshes such as SLEVE)
	\item that said, for all advection tests I'd expect SLEVE results to be somewhere between BTF and cut/slanted cells
\end{itemize}
}
...
The test was integrated forward by 6 hours using a time-step of $\Delta t = \SI{25}{\second}$ on the BTF, SLEVE, cut cell and slanted cell meshes.
\TODO{
