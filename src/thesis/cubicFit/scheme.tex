...
\end{align}
When the same analysis is performed with four cells, $\alpha_{uuu}$, $\alpha_{uu}$, $\alpha_u$ and $\alpha_d$, \rev{the stability constraint $\Mag{A} \leq 1$ gives}
\begin{align}
...
\end{align}
\rev{By considering four values for $k \Delta x$ then we obtain four constraints,}
\begin{align}
\alpha_u - \alpha_d \geq 
\begin{cases}
...
\end{align}
\rev{Guided by equations~\eqref{eqn:vonNeumann:uuConstraint} and \eqref{eqn:vonNeumann:uuuConstraint}, we choose the final stability condition}
\begin{align}
