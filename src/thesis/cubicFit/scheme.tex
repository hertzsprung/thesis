...
A local coordinate system is established \rev{that is a translation, rotation and rescaling of the physical, Cartesian coordinate system: the origin is positioned at the centroid of face $f$, with $x$ normal to the face and $y$ perpendicular to $x$, and the distance between the origin and the upwind cell centroid is one.}
The \rev{local} coordinates $(x_i, y_i)$ give the position of the centroid of the $i$th cell in the stencil.
...
We assign an integer \textit{multiplier} to each cell in the stencil, $\vect{m} = m_1 \ldots m_{12}$, and multiply equation~\eqref{eqn:cubicFit:unweightedLeastSquares} \rev{by $\vect{M} = \mathrm{diag}(\vect{m})$ to obtain}
\begin{align}
...
\end{align}
\rev{Let $y = \cos k \Delta x$ then}
\begin{align}
	\alpha_u - \alpha_d + (\alpha_{uu} - \alpha_u + \alpha_d) y - \alpha_{uu} (2 y^2 - 1) \geq 0 \text{.} \label{eq:vonNeumann:3point:y}
\end{align}
\rev{For stability, equation~\eqref{eq:vonNeumann:3point:y} should hold for $y \in [-1, 1]$.
...
\end{align}
\rev{If the minimum is in the interval $-1 < y < 1$ then the turning points of equation~\eqref{eq:vonNeumann:3point:y} can be found by differentiation such that}
\begin{align}
	\alpha_{uu} (1 - 4y) - \alpha_u + \alpha_d = 0
%
\intertext{hence}
%
	y = \frac{1}{4} - \frac{\alpha_u - \alpha_d}{4\alpha_{uu}} \text{ .}
\end{align}
\rev{Since $-1 < y < 1$ then}
\begin{align}
	- 3 \alpha_{uu} < - (\alpha_u - \alpha_d) < 5 \alpha_{uu}
%
\intertext{so that}
%
	\alpha_u - \alpha_d > -3 \alpha_{uu} \text{ and } \alpha_u - \alpha_d < 5 \alpha_{uu} \text { .}
\end{align}
\rev{Since the left-hand side of equation~\eqref{eq:vonNeumann:3point:y} is zero when $y=1$, then any minimum in the interval $-1 < y < 1$ must be less than zero, making the scheme unstable.  Hence, the minimum of equation~\eqref{eq:vonNeumann:3point:y} must occur for $y \leq 1$ or $y \geq 1$, so}
\begin{align}
	\alpha_u - \alpha_d < -3 \alpha_{uu} \text{ or } \alpha_u - \alpha_d > 5 \alpha_{uu} \label{eqn:vonNeumann:uuBounds} \text{.}
\end{align}
\rev{Guided by equations~\eqref{eqn:vonNeumann:twopoint-lower}, \eqref{eqn:vonNeumann:uuMinusOne} and \eqref{eqn:vonNeumann:uuBounds}, we choose the final stability condition}
\begin{align}
...
\rev{Inspection of stencil weights for stable and unstable numerical experiments confirms that equation~\eqref{eqn:cubicFit:stability} provides suitable stability conditions to achieve a stable transport scheme on arbitrary meshes.}
...
The stabilisation procedure \rev{uses only} the $x$ positions of these points and does not use the values of $\phi$ themselves.  The $\phi$ values are included here for illustration only.
