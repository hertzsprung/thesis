...
\end{align}
\rev{Since $-1 < y < 1$ and the quadratic equation~\eqref{eq:vonNeumann:3point:y} must have $\alpha_{uu} < 0$ for the turning point to be a minimum, then}
\begin{align}
	5 \alpha_{uu} < \alpha_u - \alpha_d < -3 \alpha_{uu} \text { .}
\end{align}
\rev{The left-hand side of equation~\eqref{eq:vonNeumann:3point:y} is zero when $y=1$, and so any minimum in the interval $-1 < y < 1$ must be less than zero, making the scheme unstable.
...
\end{align}
\rev{which is more permissive than the alternative, $\alpha_u - \alpha_d \leq 5 \alpha_{uu}$.
