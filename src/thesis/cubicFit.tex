...
Almost all of the least-squares procedure depends upon the mesh geometry only and reconstruction weights can be precomputed without knowledge of the velocity field or tracer field.
...
Instead, the computational cost of the cubicFit scheme is more comparable to dimensionally-split schemes, with the cubicFit scheme requiring only $m$ multiplies per face per time-stage.
