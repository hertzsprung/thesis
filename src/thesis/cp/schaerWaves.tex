...
\TODO{Should we test max(dt) again? we've not yet done so for a dynamical test.  I'd hope that slanted cells are much better than cut cells.}
...
Results are presented for the finite volume model with a Lorenz staggering, with momentum and potential temperature being transported using the linearUpwind scheme (figure~\ref{fig:cp:schaerWaves:w:linearUpwind}) and the cubicFit scheme (figure~\ref{fig:cp:schaerWaves:w:cubicFit}), and for the finite volume model with generalised Charney--Phillips staggering (figure~\ref{fig:cp:schaerWaves:w:cp}), and all are in general agreement with the reference solution from \citet{melvin2010}, reproduced in figure~\ref{fig:cp:schaerWaves:w:melvin}.
All four results presented in figure~\ref{fig:cp:schaerWaves:w} were all obtained using the same BTF mesh.

Spurious distortions are visible in the vertical velocity contours using the model variant with a Lorenz staggering and the linearUpwind transport scheme (figure~\ref{fig:cp:schaerWaves:w:linearUpwind}), and similar error structures have been found in previous studies and were attributed to numerical errors associated with BTF mesh distortions \citep{schaer2002,klemp2003}.
Spurious gravity wave distortions can be avoided by switching from a BTF mesh to a slanted cell mesh or cut cell mesh (results not shown).
Spurious gravity wave distortions can also be avoided by transporting momentum and potential temperature using the cubicFit scheme (figure~\ref{fig:cp:schaerWaves:w:cubicFit}).
...
Furthermore, as evidenced by the density of vertical velocity contour lines in figure~\ref{fig:cp:schaerWaves:w:cp}, the generalised Charney--Phillips model variant produces gravity wave amplitudes that are too large compared to the reference solution.
...
A less accurate solution was obtained on the BTF mesh using the generalised Charney--Phillips model variant, but all results were in good general agreement with the reference solution from \citet{melvin2010}.
Knowing that an improved transport scheme was responsible for the improved gravity wave solution using the Lorenz model variant, we might conjecture that the generalised Charney--Phillips model variant produced less accurate results because the model uses a transport scheme that is insufficiently accurate.
We return to this conjecture again in section~\ref{sec:cp:advection}, but next we will perform a further comparison between Lorenz and generalised Charney--Phillips model variants using a new test case to excite the Lorenz computational mode.
