...
The first term on the right hand side of equation~\eqref{eqn:slanted:dpdx} is the change in pressure along the terrain-following coordinate \rev{surface}, and the second term corrects for the vertical contribution in the first.
...
Cut cell meshes \rev{(also known as shaved cell meshes or embedded boundary meshes)} are less distorted than any smoothed terrain-following mesh, and some studies have shown examples where cut cells produce more accurate results when compared to terrain-following meshes \citep{good2014,steppeler2013}.
